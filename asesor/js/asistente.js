...
(function () {
  ...
  const QUESTIONS = [
    ...
    },
    {
      ...
      options: [
        { icon: 'paw',        text: 'Muy pequeño', sub: 'Hasta 5 kg',      value: 'muyPequeno' },
        { icon: 'footprints', text: 'Pequeño',      sub: 'Hasta 10 kg',    value: 'pequeno'    },
        { icon: 'weight',     text: 'Mediano',      sub: 'Hasta 25 kg',    value: 'mediano'    },
  ...
  function calcScore(answers) {
    ...
    if (answers.peso === 'muyPequeno') { s.care += 3; }
  ...
  function buildReason(plan, answers) {
    ...
    const pesoMap  = { muyPequeno:'de raza muy pequeña (hasta 5 kg)', pequeno:'de tamaño pequeño', mediano:'de tamaño mediano', grande:'de gran tamaño' };
    ...
    const parts = [edadMap[answers.edad], pesoMap[answers.peso], saludMap[answers.salud], presMap[answers.presupuesto]].filter(Boolean);
  ...
  function showResult() {
    ...
    function cardInner(pd, badgeText) {
      const chkIco = svg('<circle cx="12" cy="12" r="10"/><path d="m9 12 2 2 4-4"/>');
      return '<div class="rpc-top">' +
        '<span class="rpc-badge">' + badgeText + '</span>' +
        '<div class="rpc-pct">' + pd.pct + '</div>' +
        '<div class="rpc-pct-label">' + pd.pctLabel + '</div>' +
        '<div class="rpc-name">' + pd.name + '</div>' +
        '<div class="rpc-limit">' + pd.limit + '</div>' +
        '</div>' +
        '<ul class="rpc-features">' +
        pd.features.map(function(f){ return '<li><span class="rpc-check">' + chkIco + '</span>' + f + '</li>'; }).join('') +
        '</ul>';
    }

    var selPlan = true;
    var selRC   = !!addRCExtra;

    function renderResult() {
      var chkIcoMain = svg('<circle cx="12" cy="12" r="10"/><path d="m9 12 2 2 4-4"/>');
      ...
      resultPanel.innerHTML =
        ...
        '<div class="result-products">' +
          ...
          '<div class="result-product-card' + (selPlan ? ' selected' : '') + '" id="rpc-plan-card" style="--accent:' + pd.accent + '" tabindex="0">' +
          ...
          '<div class="result-product-card' + (selRC ? ' selected' : '') + '" id="rpc-rc-card" style="--accent:' + RC_EXTRA.accent + '" tabindex="0">' +
            '<div class="rpc-sel-row">' +
      ...
      /* ── Eventos checkboxes ── */
      function refresh() {
        var cardP = document.getElementById('rpc-plan-card');
        var cardR = document.getElementById('rpc-rc-card');
        ...
        if (cta) {
          ...
          cta.style.pointerEvents = none ? 'none' : 'auto';
          cta.href = buildURL(selPlan, selRC);
        }
      }
